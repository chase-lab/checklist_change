...
data.table::setnames(
   x = ddata,
   new = c("local",
           paste(gsub("\\.+[0-9]+", "\\.", colnames(ddata)[-1]),
                 unlist(ddata[1])[-1])
   )
)
...
env[j = c("latitude", "longitude") := data.table::tstrsplit(
   x = coords,
   split = "(?<=N) ",
   perl = TRUE)][
      j = c("longitude", "latitude") := parzer::parse_lon_lat(
...
Data presented here are fish inventories that the authors compiled at the watershed scale in British-Columbia, Canada. Compositional change between 2000 and 2005 is detailed in table 1. Data on Lampetra tridentata is not coherent between supp and table 1. The area of North and Central coast regions is computed as the sum of their main river watersheds.
Regional is British Columbia, local are river watersheds.",
