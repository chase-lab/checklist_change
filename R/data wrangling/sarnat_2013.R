...
status <- data.table::fread(
   file = "data/raw data/sarnat_2013/status.csv",
   sep = ",", encoding = "UTF-8", skip = 1, header = TRUE)
status[, species := sub("\\*", "", species)]
...
env <- data.table::fread(
   file = "data/raw data/sarnat_2013/env.csv",
   sep = ",", encoding = "UTF-8",
   header = TRUE)
