...
meta[, ":="(
   ...
   comment = "Extracted from https://doi.org/10.5285/1286b858-34a7-4ff2-84a1-a55e48d63e86. Butterflies samples obtained by trained volunteers walking along fixed transects from UK.
METHODS: 'a fixed-route walk (transect) is established at a site and butterflies are recorded along the route on a regular (weekly) basis under reasonable weather conditions for a number of years. Transect routes are chosen to sample evenly the habitat types and management activity on sites. Care is taken in choosing a transect route as it must then remain fixed to enable butterfly sightings to be compared from year to year. Transects are typically about 2-4km long, taking between 45 minutes and two hours to walk, and are divided into sections corresponding to different habitat or management units.
Butterflies are recorded in a fixed width band (typically 5m wide) along the transect each week from the beginning of April until the end of September yielding, ideally, 26 counts per year.' . Coordinates were converted to WGS84 following authors' indications:
