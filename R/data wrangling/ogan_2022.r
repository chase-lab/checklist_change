...
ddata[j = (species_columns) := lapply(
      X = .SD, FUN = function(column) replace(column, column == 0L, NA_integer_)),
...
data.table::setnames(x = ddata,
                     old = c("name_site","area","Lat","Long"),
                     new = c("local","alpha_grain","latitude","longitude"))
